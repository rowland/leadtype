...
func Main(ctx context.Context, args []string, stderr io.Writer, registerWidgets func() error) int {
	...
	fs.BoolVar(&cfg.watch, "watch", false, "watch inputs and assets for changes and rerender continuously")
	...
	normalizedArgs, err := normalizeInterspersedArgs(fs, args)
	if err != nil {
	...
	if err := run(ctx, cfg, fs.Args()); err != nil {
		fmt.Fprintf(stderr, "render-ltml: %v\n", err)
		return 1
	}
	return 0
