.PHONY: binaries test samples ltml-image-sample-local ltml-image-sample-remote
...
ltml-image-sample-remote: binaries
	...
	@set -e; \
	./$(BIN_DIR)/serve-ltml -listen $(LTML_SERVER_ADDR) -base-path . >/tmp/leadtype-serve-ltml.log 2>&1 & \
	pid=$$!; \
